...
// Google OAuth (solo si están configuradas las credenciales)
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  ...
  passport.use(new GoogleStrategy({
    ...
    try {
      ...
      const cats      = [['Alimentación','🍔','egreso'],['Transporte','🚗','egreso'],['Entretenimiento','🎬','egreso'],['Salud','💊','egreso'],['Sueldo','💰','ingreso'],['Otros','📦','ambos']];
      for (const [n,i,t] of cats) await db.query('INSERT INTO categorias (usuario_id,nombre,icono,tipo) VALUES (?,?,?,?)',[r.insertId,n,i,t]);
  ...
  passport.deserializeUser(async (id, done) => {
    const [[u]] = await require('./config/db').query('SELECT * FROM usuarios WHERE id=?',[id]);
    done(null, u);
  });
}
...
if (process.env.GOOGLE_CLIENT_ID) {
  ...
  app.get('/auth/google/callback', passport.authenticate('google', { failureRedirect:'/login' }),
    (req, res) => {
      req.session.usuario = { id:req.user.id, nombre:req.user.nombre, apellidos:req.user.apellidos, correo:req.user.correo, plan:req.user.plan, rol:req.user.rol };
